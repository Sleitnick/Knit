The [RemoteProperty](https://github.com/AtollStudios/Knit/blob/main/src/Util/Remote/RemoteProperty.lua) module wraps a ValueBase object to expose properties to the client from the server. The server can read and write to this object, but the client can only read. This is useful when it's overkill to write a combination of a method and event to replicate data to the client.
...
## [ClientRemoteProperty](https://github.com/AtollStudios/Knit/blob/main/src/Util/Remote/ClientRemoteProperty.lua)

The [ClientRemoteProperty](https://github.com/AtollStudios/Knit/blob/main/src/Util/Remote/ClientRemoteProperty.lua) module wraps a ValueBase object to expose properties from the server to the client. The client can only read the value. This class should be used alongside RemoteProperty on the server.
