...
local function BuildService(serviceName, folder)
	...
	if older:FindFirstChild("RF" then
...
function KnitClient.CreateController(controller)
	...
	assert(not KnitClient.Controllers[controller.Name], "Controller \"" .. controller.Name .. "\" already exists")
...
function KnitClient.Start()
	...
	return Promise.new(function(resolve)
		...
		for _,controller in pairs(controllers) do
			if typeof(controller.KnitInit) == "function" then
		...
		-- Start:
		for _,controller in pairs(controllers) do
			if typeof(controller.KnitStart) == "function" then
